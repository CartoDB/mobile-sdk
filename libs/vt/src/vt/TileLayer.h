...
namespace carto { namespace vt {
    class TileLayer {
    public:
        explicit TileLayer(int layerIdx, float opacity, boost::optional<CompOp> compOp, std::vector<std::shared_ptr<TileBitmap>> bitmaps, std::vector<std::shared_ptr<TileGeometry>> geometries, std::vector<std::shared_ptr<TileLabel>> labels) : _layerIdx(layerIdx), _opacity(opacity), _compOp(std::move(compOp)), _bitmaps(std::move(bitmaps)), _geometries(std::move(geometries)), _labels(std::move(labels)) { }
