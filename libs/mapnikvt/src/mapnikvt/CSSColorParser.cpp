...
namespace carto { namespace mvt {
    bool parseCSSColor(std::string name, unsigned int& value) {
        ...
        name = toLower(name);
        auto begin = &colorEntries[0];
        auto end = &colorEntries[0] + sizeof(colorEntries) / sizeof(ColorEntry);
        auto it = std::lower_bound(begin, end, name, [](const ColorEntry& entry, const std::string& entryName) {
            return entry.name < entryName;
        });
        if (it != end) {
            if (it->name == name) {
                value = it->value | 0xff000000;
                return true;
            }
        }
        if (name == "transparent") {
        ...
        if (name.substr(0, 1) == "#") {
            std::string code(name.begin() + 1, name.end());
